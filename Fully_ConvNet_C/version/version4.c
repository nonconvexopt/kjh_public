...
double ***conv_forward(conv *layer, double ***input) {
    int out_ch, out_w, out_h, in_ch, fil_w, fil_h, pos_w, pos_h;

    int lim_out_ch = layer->output_shape[0];
    int lim_out_w = layer->output_shape[1];
    int lim_out_h = layer->output_shape[2];
    ...
    double ***output = (double ***)calloc(lim_out_ch, sizeof(double **));

    //Matrix Multiplty
    for (out_ch = 0; out_ch < lim_out_ch; out_ch++) {
        output[out_ch] = (double **)calloc(lim_out_w, sizeof(double *));
        for (out_w = 0; out_w < lim_out_w; out_w++) {
            output[out_ch][out_w] = (double *)calloc(lim_out_h, sizeof(double));
            w_offset = out_w * receptive_field_w - (lim_fil_w / 2);
            for (out_h = 0; out_h < lim_out_h; out_h++) {
                h_offset = out_h * receptive_field_h - (lim_fil_h / 2);
                result = 0.0;
                        ...
                        for (fil_h = 0; fil_h < lim_fil_h; fil_h++) {
                            ...
                            if ((0 <= pos_w) && (pos_w < lim_in_w) &&
                                (0 <= pos_h) && (pos_h < lim_in_h)) {
                                    ...
                                    layer->filter[in_ch][out_ch][fil_w][fil_h];
                            }
                        }
                    }
                }
                result += layer->bias[out_ch];
                if (result < 0.0) {
                    result *= 0.01;
                }
                output[out_ch][out_w][out_h] = result;

                //calculating gradients
                if (result > 0) {
                    layer->output_grad[out_ch][out_w][out_h] = 1.0;
                } else {
                    layer->output_grad[out_ch][out_w][out_h] = 0.01;
                }
            }
        }
    }
...
void update(conv **params, softmax *out, double ****forwards, int depth, double learning_rate, double l2_penalty) {
    int layer;
    int in_ch, in_w, in_h, out_ch, out_w, out_h, fil_w, fil_h;
    int lim_in_ch, lim_in_w, lim_in_h, lim_out_ch, lim_out_w, lim_out_h, lim_fil_w, lim_fil_h;
    int offset_w, offset_h, pos_w, pos_h, receptive_field_w, receptive_field_h;
    ...
    lim_in_ch = out->input_dim;
    temp_weight_grad = (double **)calloc(lim_in_ch, sizeof(double *));
    for (in_ch = 0; in_ch < lim_in_ch; in_ch++) {
        temp_weight_grad[in_ch] = (double *)calloc(lim_in_ch, sizeof(double));
    }

    lim_in_ch = params[depth - 1]->output_shape[0];
    lim_in_w = params[depth - 1]->output_shape[1];
    lim_in_h = params[depth - 1]->output_shape[2];

    temp_input_grad = (double ***)calloc(lim_in_ch, sizeof(double **));
    for (in_ch = 0; in_ch < lim_in_ch; in_ch++) {
        temp_input_grad[in_ch] = (double **)calloc(lim_in_w, sizeof(double *));
        for (in_w = 0; in_w < lim_in_w; in_w++) {
            temp_input_grad[in_ch][in_w] = (double *)calloc(lim_in_h, sizeof(double));
        }
    }
    ...
    for (layer = depth - 1; layer > 0; layer--) {
        lim_in_ch = params[layer]->input_shape[0];
        lim_in_w = params[layer]->input_shape[1];
        lim_in_h = params[layer]->input_shape[2];
        
        lim_fil_w = params[layer]->filter_size[2];
        lim_fil_h = params[layer]->filter_size[3];

        lim_out_ch = params[layer]->output_shape[0];
        lim_out_w = params[layer]->output_shape[1];
        lim_out_h = params[layer]->output_shape[2];


        //Making temporal storage
        temp_filter_grad = (double ****)calloc(lim_in_ch, sizeof(double ***));
        for (in_ch = 0; in_ch < lim_in_ch; in_ch++) {
            temp_filter_grad[in_ch] = (double ***)calloc(lim_out_ch, sizeof(double **));
            for (out_ch = 0; out_ch < lim_out_ch; out_ch++) {
                temp_filter_grad[in_ch][out_ch] = (double **)calloc(lim_fil_w, sizeof(double *));
                for (fil_w = 0; fil_w < lim_fil_w; fil_w++) {
                    temp_filter_grad[in_ch][out_ch][fil_w] = (double *)calloc(lim_fil_h, sizeof(double));
                }
            }
        }
        ...
        temp_bias_grad = (double *)calloc(lim_out_ch, sizeof(double));

        receptive_field_w = params[layer]->stride_width + 1;
        receptive_field_h = params[layer]->stride_height + 1;
    ...
    lim_in_ch = params[0]->input_shape[0];
    lim_in_w = params[0]->input_shape[1];
    lim_in_h = params[0]->input_shape[2];
        
    lim_fil_w = params[0]->filter_size[2];
    lim_fil_h = params[0]->filter_size[3];

    lim_out_ch = params[0]->output_shape[0];
    lim_out_w = params[0]->output_shape[1];
    lim_out_h = params[0]->output_shape[2];
    ...
    //Calculating per-layer backpropagation
    receptive_field_w = params[0]->stride_width + 1;
    receptive_field_h = params[0]->stride_height + 1;
